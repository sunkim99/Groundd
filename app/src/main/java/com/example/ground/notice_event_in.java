...
//공지 게시판 !!이벤트!! 에서 게시물을 눌렀을때 해당게시물 보여지는화면

public class notice_event_in extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_notice_event_in);
        ...
        Response.Listener<String> responseListener = new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (success) {
                        Log.d("TEST1234", "[공지_이벤트] 쓰레드확인");

                        String number = jasonObject.getString("annNum");
                        Log.d("TEST1234", "[공지_이벤트] 쓰레드확인1 ");
                        Log.d("TEST1234", "[공지_이벤트] 가져온 글번호 : " + number);


                        String userID = jasonObject.getString("userID");
                        Log.d("TEST1234", "[공지_이벤트] 아이디 : " + userID);

                        String annTi = jasonObject.getString("annTi");
                        Log.d("TEST1234", "[공지_이벤트] 제목 : " + annTi);
                        String annCon = jasonObject.getString("annCon");

                        String annDate = jasonObject.getString("annDate");

                        Log.d("TEST1234", "[공지_이벤트] 쓰레드확인2");
                        ...
                        Log.d("TEST1234", "[공지_이벤트] 공지 가져오기 실패");
                        return;
                    }
        ...
        RequestQueue queue = Volley.newRequestQueue(notice_event_in.this);
        queue.add(nndr);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.delete) {
            AlertDialog.Builder builder = new AlertDialog.Builder(notice_event_in.this);
            builder.setTitle("이벤트 삭제");
            builder.setMessage("해당 이벤트를 삭제하시겠습니까?");
            builder.setPositiveButton("예", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    Response.Listener<String> admin_notice1 = new Response.Listener<String>() {
                        ...
                        public void onResponse(String response) {
                            try {
                                ...
                                boolean success = jasonObject.getBoolean("success");
                                if (success) {
                                    Log.d("TEST1234", "[공지_이벤트 삭제하기] 쓰레드확인");
                                    String ssss = jasonObject.getString("str");
                                    Log.d("TEST1234", "[공지_이벤트 삭제하기] 수행될 쿼리문 :" + ssss);

                                    Log.d("TEST1234", "[공지_이벤트 삭제하기] 쓰레드확인2:");

                                } else {
                                    Log.d("TEST1234", "[공지_이벤트 삭제하기] 게시글 정보오류 ");
                                    return;
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                    ...
                    Intent intent = new Intent(notice_event_in.this, notice_event.class); //화면 어디로 넘어가는지 확인하기
                    ...
                    RequestQueue comment_queue = Volley.newRequestQueue(notice_event_in.this);
        ...
        if (v.getId() == R.id.top_navi) { //상단바 클릭
            Intent intent1 = new Intent(notice_event_in.this, MainActivity.class);
